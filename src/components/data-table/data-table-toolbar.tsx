...
export function DataTableToolbar<TData>({ table, children, className, ...props }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div
      ...
      className={cn("flex w-full items-start justify-end gap-2 p-1", className)}
      {...props}
    >
...
function DataTableToolbarFilter<TData>({ column }: DataTableToolbarFilterProps<TData>) {
  {
    ...
    const onFilterRender = React.useCallback(() => {
      ...
      switch (columnMeta.variant) {
        case "text":
          return (
            <div className="border rounded-full p-0.5 pr-4 flex items-center justify-between">
              <Input
                placeholder={columnMeta.placeholder ?? columnMeta.label}
                value={(column.getFilterValue() as string) ?? ""}
                onChange={(event) => column.setFilterValue(event.target.value)}
                className="h-8 w-40 lg:w-44 md:text-xs focus-visible:outline-none border-none shadow-none focus-visible:ring-0"
              />
              <SearchIcon />
            </div>
          )
